...
VO is a framework that helps to resolve various problems faced by the world wide astronomical community. 
One of this problems is related to have access to data generated by differents instruments. Therefore an architecture 
\cite{ivoaarchitecture} was designed by IVOA, which is composed by standards and protocols, that allow the transparent 
and unified access to astronomical data servers. The IVOA Architecture define 3 layers:
\begin{itemize}
	\item \textbf{Resource Layer}: it consist in a collection of servers with data from differents instruments.
	\item \textbf{User Layer}: astronomers and researchers looking for consume data through differents means.
	\item \textbf{Middle Layer}: this layer allow to connect the previous two layer in a transparent way for the users, they can find data through the registry, or get data through the data acccess protocols. 
\end{itemize}
